...
declare
  ...
  v_open  boolean;
begin
  select (value->>'max_users')::int into v_max from public.system_settings where key = 'max_users';
  select (value->>'registration_open')::boolean into v_open from public.system_settings where key = 'registration_open';
